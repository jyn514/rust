    ...
    fn clean(&self, cx: &mut DocContext<'_>) -> ExternalCrate {
        ...
        let primitives = if root.is_local() {
            tcx.hir()
                ...
                .iter()
                .filter_map(|&id| {
                    let item = tcx.hir().expect_item(id.id);
                    match item.kind {
                        hir::ItemKind::Mod(_) => as_primitive(Res::Def(
                            DefKind::Mod,
                            tcx.hir().local_def_id(id.id).to_def_id(),
                        )),
                            ...
                            if item.vis.node.is_pub() =>
                        {
                            as_primitive(path.res).map(|(_, prim)| {
                                ...
                                (tcx.hir().local_def_id(id.id).to_def_id(), prim)
                            })
                        }
                        _ => None,
                    }
            ...
            tcx.hir()
                ...
                .filter_map(|&id| {
                    ...
                    match item.kind {
                        ...
                        {
                            as_keyword(path.res)
                                .map(|(_, prim)| (tcx.hir().local_def_id(id.id).to_def_id(), prim))
                        }
                        _ => None,
                    }
                })
...
impl Clean<Item> for (&hir::ForeignItem<'_>, Option<Symbol>) {
    fn clean(&self, cx: &mut DocContext<'_>) -> Item {
        let (item, renamed) = self;
        cx.with_param_env(cx.tcx.hir().local_def_id(item.hir_id).to_def_id(), |cx| {
            let kind = match item.kind {
