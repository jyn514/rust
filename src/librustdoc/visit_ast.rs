...
impl<'a, 'tcx> RustdocVisitor<'a, 'tcx> {
    ...
    crate fn visit(mut self, krate: &'tcx hir::Crate<'_>) -> Module<'tcx> {
        ...
        self.cx.cache.borrow_mut().exact_paths = self.exact_paths;
        top_level_module
    }
    ...
    fn maybe_inline_local(
            ...
            if !self_is_hidden {
                if let Res::Def(kind, did) = res {
                    if kind == DefKind::Mod {
                        ...
                        self.cx.cache.get_mut().access_levels.map.insert(did, AccessLevel::Public);
                    }
                }
            }
            return false;
        ...
        let is_private = !self.cx.cache.borrow().access_levels.is_public(res_did);
