...
impl Config {
    ...
    pub fn parse(args: &[String]) -> Config {
        ...
        config.stage = match config.cmd {
            ...
            Subcommand::Doc { .. } => flags.stage.or(build.doc_stage).unwrap_or(if download_rustc { 2 } else { 0 }),
            Subcommand::Build { .. } => flags.stage.or(build.build_stage).unwrap_or(if download_rustc { 2 } else { 1 }),
            Subcommand::Test { .. } => flags.stage.or(build.test_stage).unwrap_or(if download_rustc { 2 } else { 1 }),
